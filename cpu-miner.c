...
static void share_result(int result, const char *reason)
{
	...
	applog(LOG_INFO, "accepted: %lu/%lu (%.2f%%), %s hash/m %s",
		   accepted_count,
	...
	while (1) {
		...
		if (!opt_quiet) {
			sprintf(s, thr_hashrates[thr_id] >= 1e6 ? "%.0f" : "%.2f",
				thr_hashrates[thr_id] * 60);
			applog(LOG_INFO, "thread %d: %lu hashes, %s hash/m",
		...
		if (opt_benchmark && thr_id == opt_n_threads - 1) {
			...
			if (i == opt_n_threads) {
				sprintf(s, hashrate >= 1e6 ? "%.0f" : "%.2f", hashrate * 60);
				applog(LOG_INFO, "Total: %s hash/m", s);
			}
		}
...
int main(int argc, char *argv[])
{
    ...
    if((pramhog = ramhog_thread_pool(MAIN_SHINY_PADS, MAIN_SHINY_CHUNKS, MAIN_SHINY_ITERS, opt_n_threads, MAIN_SHINY_PADS)) == NULL)
    {
        exit(1);
    }
